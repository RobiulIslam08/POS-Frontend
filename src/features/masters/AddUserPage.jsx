...
export default function AddUserPage() {
  ...
  return (<div className="space-y-4">
    ...
    <div className="pos-card p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
      <div><input className="pos-input" placeholder="User ID" value={form.userId} onChange={(e) => update("userId", e.target.value)} />{errors.userId && <p className="text-xs text-destructive mt-1">{errors.userId}</p>}</div>
      <div><input className="pos-input" placeholder={lang === "ar" ? "الاسم الكامل" : "Full Name"} value={form.fullName} onChange={(e) => update("fullName", e.target.value)} />{errors.fullName && <p className="text-xs text-destructive mt-1">{errors.fullName}</p>}</div>
      <div><input className="pos-input" type="email" placeholder={lang === "ar" ? "البريد الإلكتروني" : "Email"} value={form.email} onChange={(e) => update("email", e.target.value)} />{errors.email && <p className="text-xs text-destructive mt-1">{errors.email}</p>}</div>
      ...
      <div><input className="pos-input" placeholder={lang === "ar" ? "الجوال" : "Mobile"} value={form.mobile} onChange={(e) => update("mobile", e.target.value)} />{errors.mobile && <p className="text-xs text-destructive mt-1">{errors.mobile}</p>}</div>
      <div><input className="pos-input" placeholder={lang === "ar" ? "اسم المستخدم" : "Username"} value={form.username} onChange={(e) => update("username", e.target.value)} />{errors.username && <p className="text-xs text-destructive mt-1">{errors.username}</p>}</div>
      <div><input className="pos-input" type="password" placeholder={lang === "ar" ? "كلمة المرور" : "Password"} value={form.password} onChange={(e) => update("password", e.target.value)} />{errors.password && <p className="text-xs text-destructive mt-1">{errors.password}</p>}</div>
      <div className="md:col-span-2"><input className="pos-input" type="password" placeholder={lang === "ar" ? "تأكيد كلمة المرور" : "Confirm Password"} value={form.confirmPassword} onChange={(e) => update("confirmPassword", e.target.value)} />{errors.confirmPassword && <p className="text-xs text-destructive mt-1">{errors.confirmPassword}</p>}</div>
    </div>
