    ...
    <div className="pos-card overflow-x-auto">
      <table className="pos-table">
        ...
        <tbody>
          ...
          : purchases.map((p, i) => <tr key={p._id}><td>{i + 1}</td><td>{p.invoiceNo}</td><td>{new Date(p.invoiceDate).toLocaleDateString()}</td><td>{p.supplier?.supplierName || "—"}</td><td>{p.purchaseOrderNo || "—"}</td><td>{p.totalAmount}</td><td><button className="text-primary text-sm">View</button></td></tr>)}
        </tbody>
      </table>
    </div>
  </div>);
