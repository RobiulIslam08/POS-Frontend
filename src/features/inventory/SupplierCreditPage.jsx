...
export default function SupplierCreditPage() {
  ...
  const remaining = useMemo(() => {
    const due = Number(form.dueAmount) || 0;
    const paid = Number(form.paidAmount) || 0;
    return (due - paid).toFixed(2);
  }, [form.dueAmount, form.paidAmount]);
  ...
  const handleRecord = () => {
    ...
    setPayments((prev) => [
      {
        id: `${form.invoiceNo}-${prev.length + 1}`,
        supplier: form.supplier,
        invoiceNo: form.invoiceNo,
        paidAmount: form.paidAmount,
        paymentDate: form.paymentDate,
        method: form.paymentMethod,
        status: canSettle ? "Settled" : "Pending Approval",
      },
      ...prev,
    ]);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="pos-card p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <div className="md:col-span-2 text-xs text-muted-foreground rounded-md border border-border px-3 py-2 bg-accent/20">
          {canSettle
            ? (lang === "ar" ? "يمكنك تسوية رصيد المورد" : "You can settle supplier outstanding")
            : (lang === "ar" ? "يمكنك تسجيل الدفع فقط بانتظار اعتماد الإدارة" : "You can record payment only, pending management approval")}
        </div>
      </div>

      <div className="pos-card p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div>
          <input className="pos-input" placeholder={lang === "ar" ? "المورد" : "Supplier"} value={form.supplier} onChange={(e) => update("supplier", e.target.value)} />
          {errors.supplier && <p className="text-xs text-destructive mt-1">{errors.supplier}</p>}
        </div>
        <div>
          <input className="pos-input" placeholder="Invoice No" value={form.invoiceNo} onChange={(e) => update("invoiceNo", e.target.value)} />
      ...
      <div className="pos-card p-4 md:p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <input className="pos-input" placeholder="Due Amount" type="number" value={form.dueAmount} onChange={(e) => update("dueAmount", e.target.value)} />
          {errors.dueAmount && <p className="text-xs text-destructive mt-1">{errors.dueAmount}</p>}
        </div>
        <div>
          <input className="pos-input" placeholder="Paid Amount" type="number" value={form.paidAmount} onChange={(e) => update("paidAmount", e.target.value)} />
        ...
        <div className="md:col-span-2 flex justify-center gap-3">
          <button className="pos-btn-primary gap-1" onClick={handleRecord}><Save size={14} /> {lang === "ar" ? "تسجيل الدفع" : "Record Payment"}</button>
      ...
      <div className="pos-card overflow-x-auto">
        <table className="pos-table min-w-[900px]">
          ...
          <tbody>
            ...
            {payments.map((p) => <tr key={p.id}><td>{p.supplier}</td><td>{p.invoiceNo}</td><td>SR {p.paidAmount}</td><td>{p.paymentDate}</td><td>{p.method}</td><td>{p.status}</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>
